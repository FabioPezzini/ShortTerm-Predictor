...
dataTab = data {:,:};
dataTabOutlier = filloutliers(dataTab(isoutlier(dataTab(:,3)),:), 'center');
dataTabCp1 = dataTab(1:365,:);
dataTabCp2 = dataTab(366:730,:);
meanY1 = nanmean(dataTabCp1(:,3));
meanY2 = nanmean(dataTabCp2(:,3));
meanSetOutlier475 = mean([dataTab(475-7,3), dataTab(475+7,3)]);
meanSetOutlier476 = mean([dataTab(476-7,3), dataTab(476+7,3)]);
...
figure(199)
subplot(2,2,1)
plot(dataTabCp1(:,1), dataTabCp1(:,3)-trend1);
hold on
plot(dataTabCp1(:,1),trend1)
subplot(2,2,2)
plot(dataTabCp2(:,1), dataTabCp2(:,3)-trend2);
hold on
...
%settimana detrendizzata per fourier
...
thetaCapFour = phiFour \ y1D;
misuraStimataFour = phiFour * thetaCapFour + meanFourier;
scartoFour = y1D - misuraStimataFour;
SSRFOUR = scartoFour' * scartoFour;

phiFourVAL = ones(size(mer2D));
for n = 1:2
    phiFourVAL = [phiFourVAL, cos(n*w.*mer2D),sin(n*w.*mer2D),cos(n*w.*gio2D),sin(n*w.*gio2D),cos(n*w.*ven2D),sin(n*w.*ven2D),cos(n*w.*sab2D),sin(n*w.*sab2D),cos(n*w.*dom2D),sin(n*w.*dom2D),cos(n*w.*lun2D),sin(n*w.*lun2D),cos(n*w.*mar2D),sin(n*w.*mar2D)];
end
misuraStimataFourVAL = phiFourVAL * thetaCapFour + meanFourier;
scartoFourVAL = y2D - misuraStimataFourVAL;
SSRFOURVAL = scartoFourVAL' * scartoFourVAL;

figure (4);
subplot(2,1,1);
scatter(settimane,y1D,'r','o');
hold on;
...
scartoFour70 = y1D - misuraStimataFour70;
...
thetaCapFour84 = phiFour84 \ y1D;
misuraStimataFour84 = phiFour84 * thetaCapFour84 + meanFourier;
scartoFour84 = y1D - misuraStimataFour84;
...
thetaCapFour98 = phiFour98 \ y1D;
misuraStimataFour98 = phiFour98 * thetaCapFour98 + meanFourier;
scartoFour98 = y1D - misuraStimataFour98;
...
thetaCapFour112 = phiFour112 \ y1D;
misuraStimataFour112 = phiFour112 * thetaCapFour112 + meanFourier;
scartoFour112 = y1D - misuraStimataFour112;
...
mdlFOUR= (log(length(misure))*length(thetaCapFour))/length(misure) + log(SSRFOUR);
...
mdlFOURVAL = (log(length(misure))*length(thetaCapFour))/length(misure) + log(SSRFOURVAL);
